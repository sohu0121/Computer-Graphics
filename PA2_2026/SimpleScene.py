...
# ===== Project 2 : roller-coaster state =====
controlPoints = []  # list of cow2wld matrices fixed at each clicked control point
isAnimating = False  # True while the cow runs along the spline track
animStartTime = 0.0  # glfw.get_time() captured when the animation starts
ANIM_SPEED = 1.5  # spline-parameter units advanced per second (tune to taste)

# cubic uniform B-spline basis matrix:  C(t) = [t^3 t^2 t 1] . M . [P0 P1 P2 P3]^T
BSPLINE_M = (1.0 / 6.0) * np.array([
    [-1.0, 3.0, -3.0, 1.0],
    [3.0, -6.0, 3.0, 0.0],
    [-3.0, 0.0, 3.0, 0.0],
    [1.0, 4.0, 1.0, 0.0]])
...
def splinePoint(pts, u, deriv=False):
    # Evaluate a CYCLIC cubic B-spline (approximating spline).
    # pts: list of control-point positions (each a length-3 vector)
    # u in [0, n): integer part selects the segment, fractional part is t in [0,1).
    # deriv=True returns the tangent (1st derivative) instead of the position.
    n = len(pts)
    base = int(np.floor(u))
    seg = base % n
    t = u - base
    # segment uses 4 consecutive control points, wrapping around (cyclic)
    G = np.array([pts[(seg + i) % n] for i in range(4)])  # 4 x 3
    if deriv:
        tv = np.array([3.0 * t * t, 2.0 * t, 1.0, 0.0])
    else:
        tv = np.array([t ** 3, t ** 2, t, 1.0])
    return tv @ BSPLINE_M @ G


def cowFrame(pos, tangent):
    # Build a cow2wld matrix that places the cow at pos and orients it so that
    # it faces along the tangent. Because the tangent carries a vertical (y)
    # component, this yields the yaw AND the pitch at the same time.
    front = normalize(tangent)
    worldUp = np.array((0.0, 1.0, 0.0))
    side = np.cross(worldUp, front)
    if np.linalg.norm(side) < 1e-8:  # tangent is (nearly) vertical
        side = np.array((1.0, 0.0, 0.0))
    side = normalize(side)
    up = normalize(np.cross(front, side))
    M = np.eye(4)
    # Map the cow's local axes to the travel frame.
    # The cow model travels nose-first along its local +x axis here; if the cow
    # ends up facing the wrong way, swap which column gets `front`/`side`
    # (or negate `front`) until the head points along the track.
    M[0:3, 0] = front
    M[0:3, 1] = up
    M[0:3, 2] = side
    M[0:3, 3] = pos
...
def display():
    ...
    global controlPoints, isAnimating, animStartTime
    ...
    # Draw a duplicated cow at every control point that has been specified.
    for cp in controlPoints:
        drawCow(cp, False)

    # Animate the cow along the cyclic B-spline track (three full laps).
    if isAnimating:
        u = (glfw.get_time() - animStartTime) * ANIM_SPEED
        n = len(controlPoints)  # 6 control points -> 6 cyclic segments
        if u >= n * 3.0:  # finished three laps
            isAnimating = False
            controlPoints = []  # return to the initial cursor-following mode
        else:
            pts = [getTranslation(M) for M in controlPoints]
            pos = splinePoint(pts, u, deriv=False)  # position on the curve
            tan = splinePoint(pts, u, deriv=True)  # tangent -> heading (yaw+pitch)
            cow2wld = cowFrame(pos, tan)
...
def onMouseButton(window, button, state, mods):
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        if state == GLFW_DOWN:
            if isDrag == H_DRAG:
                isDrag = 0
                # The cow has been positioned; fix a control point here and
                # leave a duplicated cow at the clicked location.
                if (not isAnimating) and len(controlPoints) < 6:
...
def onMouseDrag(window, x, y):
    ...
    if isDrag:
        ...
        if isDrag == V_DRAG:
            # vertical dragging: move the cow only along the world y-axis.
            if cursorOnCowBoundingBox:
                ray = screenCoordToRay(window, x, y)
                pp = pickInfo
                # dragging plane perpendicular to the viewing ray, through the pick point
                normal = normalize(getTranslation(cam2wld[cameraIndex]) - pp.cowPickPosition)
                p = Plane(normal, pp.cowPickPosition)
                c = ray.intersectsPlane(p)
                currentPos = ray.getPoint(c[1])

                newPos = pp.cowPickPosition.copy()
                newPos[1] = currentPos[1]  # keep x,z ; change only the height
                T = np.eye(4)
                setTranslation(T, newPos - pp.cowPickPosition)
